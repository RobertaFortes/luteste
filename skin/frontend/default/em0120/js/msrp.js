...
Catalog.Map = {
    ...
    showHelp: function(event) {
        ...
        if (this != Catalog.Map && Catalog.Map.active != this.link) {
			...
			//fix show popup show wrong 
			var temp = event.pageX - (Element.getWidth(helpBox) / 2) ;
			if(temp < 0)
				helpBox.style.left = "5px";
			else
				if(temp + Element.getWidth(helpBox) > Element.getWidth(bodyNode))
					helpBox.style.right = "5px";
				else
					helpBox.style.left = temp + 'px';		
