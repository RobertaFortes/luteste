<?php
class EM_Filterproducts_Block_List extends Mage_Catalog_Block_Product_Abstract implements Mage_Widget_Block_Interface
{
	const BEST_SELLER_CACHE_TAG = 'ft_bestseller';
    ...
    protected function _toHtml()
	{
		if($this->getData('choose_template')	==	'custom_template')
		{
			if($this->getData('custom_theme'))
				...
				$this->setTemplate('em_filterproducts/sample_custom.phtml');
		}
		else
		{
	...
	public function getShowFrontend($attr){
		if(strpos($this->getData('show'),$attr) !== false)	return true;
		else return false;
	}
    ...
    protected function getProductCollection(){
		...
		$type = $this->getData('type_filter');
			if($type == 1)		$products	= $this->getFilterFeatured($products);		//	Special Attribute
			elseif($type == 2)	$products	= $this->getFilterBest($products);			//	Bestseller Product
			elseif($type == 3)	$products	= $this->getFilterNew($products);			//	New Products
			elseif($type == 4)	$products	= $this->getFilterSale($products);			//	Sales Products
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection
     */
    protected function getFilterNew($products){			//	New Products
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection
     */
    protected function getFilterSale($products){		//	Sales Products
		$todayStartOfDayDate  = Mage::app()->getLocale()->date()
            ->setTime('00:00:00')
            ->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection
     */
    protected function getFilterBest($products){		//	Bestseller Product
     ...
     * @return Mage_Catalog_Model_Resource_Product_Collection
     */
    protected function filterCate($products){		//Filter by categories
		$config = $this->getData('new_category');
		if($config){
			$result = array();
			$condition_cat = array();
			$alias = 'cat_index';
