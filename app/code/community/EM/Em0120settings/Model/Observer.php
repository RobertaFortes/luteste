<?php
class EM_Em0120settings_Model_Observer {
	
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'em0120')){	
			# Disable default magento navigation
			if (Mage::helper('em0120settings')->getGeneral_DisableDefaultNav()&&(Mage::getConfig()->getModuleConfig('EM_Megamenupro')->is('active', 'true'))) {
				$blocks = $observer->getLayout()->getXpath('//block[@name="em0120.catalog.topnav"]');
				if (!empty($blocks))
					$blocks[0]->addAttribute('ignore', true);
			}
		
			# Disable EM variation module on frontend
			if (Mage::helper('em0120settings')->getGeneral_DisableFrontendVariation()) {
			...
			if (Mage::helper('em0120settings')->getGeneral_DisableFooterLinks()) {
