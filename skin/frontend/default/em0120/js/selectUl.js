(function($) {
	...
	$.fn.insertUlCurrency = function() {
		var numOptions = $(this).children().length;
		$('<div class="toolbar-dropdown"><span class="current"/><ul style="display:none" /></div>').insertAfter($(this).toggleClass('.toolbar-switch').parent());
		var divSpan = $(this).toggleClass('toolbar-switch').parent().parent().find('span');
		divSpan.append($(this).parent().children('select').find('option:selected').text());
		imageUrl = currency + ($(this).find('option').eq(i).text()).replace(/\s/g,'').toLowerCase() + '.png';
		divSpan.css('background-image', 'url(' + currency + ($(this).parent().children('select').find('option:selected').text()).replace(/\s/g,'').toLowerCase() + '.png)');
		divSpan.css('background-repeat', 'no-repeat');
		var divUl = $(this).toggleClass('toolbar-switch').parent().parent().find('ul');
		for (var i = 0; i < numOptions; i++) {
			var text = '<li><a style="background-image:url(' + currency + ($(this).find('option').eq(i).text()).replace(/\s/g,'').toLowerCase() + '.png); background-repeat: no-repeat;" href ="' + $(this).find('option').eq(i).val() + '">' + $(this).find('option').eq(i).text() + '</a></li>';
			divUl.append(text);
		}
	};
